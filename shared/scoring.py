...
if __name__ == "__main__":
    # Self-test with a clearly bot-inflated profile and a clean one.
    import json

    botty = {
        "platform": "instagram", "tier_size": 250_000,
        "engagement": {"avg_er_pct": 0.2, "base": "followers", "spike_count": 4, "max_spike_ratio": 12, "sustained_drop": True},
        "ratios": {"like_to_comment": 320, "following_to_follower": 0.1, "sub_to_view_pct": None},
        "audience": {"sample_size": 200, "suspicious_pct": 47},
        "comments": {"sampled": 80, "generic_pct": 62, "emoji_only_pct": 41, "duplicate_pct": 18, "suspicious_author_pct": 55},
        "history": {"age_days": 200, "posts_per_day": 1.2, "default_avatar": False, "verified": False},
    }
    clean = {
        "platform": "youtube", "tier_size": 120_000,
        "engagement": {"avg_er_pct": 2.4, "base": "views", "spike_count": 0, "max_spike_ratio": 1.8, "sustained_drop": False},
        "ratios": {"like_to_comment": 35, "following_to_follower": None, "sub_to_view_pct": 22},
        "audience": None,
        "comments": {"sampled": 90, "generic_pct": 12, "emoji_only_pct": 8, "duplicate_pct": 1, "suspicious_author_pct": 5},
        "history": {"age_days": 1500, "posts_per_day": 0.3, "default_avatar": False, "verified": True},
    }
    for label, s in [("BOT-INFLATED IG", botty), ("CLEAN YT", clean)]:
        r = score(s)
        print(f"\n=== {label} ===")
        print(f"Score {r['authenticity_score']}/100 — {r['verdict_emoji']} {r['verdict']}")
        print("Deductions:", json.dumps(r["deductions"], indent=2))
